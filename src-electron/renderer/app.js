...
function renderCategories() {
  ...
  categories.forEach((cat) => {
    ...
    btn.className = `cat-tab${cat.name === currentCategory ? ' active' : ''}`;
...
function renderItems() {
  ...
  if (currentCategory === '__all__') {
    ...
    catOrder.forEach((cat) => {
      ...
      if (catColor) sep.style.borderLeft = `3px solid ${catColor}`;
...
function createCard(item) {
  ...
  } else if (item.type === 'executable' || item.type === 'console') {
    const def = document.createElement('div');
    def.className = 'item-icon-default';
    def.style.background = hashColor(item.name);
    def.textContent = item.name.charAt(0).toUpperCase();
    iconWrap.appendChild(def);
    loadExeIcon(item, def);
  } else {
    const def = document.createElement('div');
    def.className = 'item-icon-default';
    def.style.background = hashColor(item.name);
...
async function loadExeIcon(item, defaultEl) {
  const exePath = item.type === 'console' ? null : item.path_or_url;
  if (!exePath) return;
  try {
    const dataUrl = await api.getFileIcon(exePath);
    if (!dataUrl) return;
    const img = document.createElement('img');
    img.className = 'item-icon';
    img.src = dataUrl;
    img.draggable = false;
    defaultEl.parentNode.replaceChild(img, defaultEl);
  } catch {
  }
}
...
const catListManage = document.getElementById('cat-list-manage');
...
catModalOverlay.addEventListener('click', (e) => {
  if (e.target === catModalOverlay) closeCategoryManager();
});
...
function renderCategoryManagerList() {
  ...
  categories.forEach((cat) => {
    ...
    const dot = document.createElement('span');
    dot.className = 'cat-mgr-dot';
    dot.style.background = cat.color;

    const nameSpan = document.createElement('span');
    nameSpan.className = 'cat-mgr-name';
    nameSpan.textContent = cat.name;

    const colorInput = document.createElement('input');
    colorInput.type = 'color';
    colorInput.className = 'cat-mgr-color';
    colorInput.value = cat.color;
    colorInput.addEventListener('input', async () => {
      try {
        await api.updateCategory(cat.name, { color: colorInput.value });
    ...
    delBtn.className = 'btn-icon cat-mgr-del';
    delBtn.textContent = '✕';
    delBtn.title = 'Delete category';
    delBtn.addEventListener('click', async () => {
      if (!confirm(`Delete category "${cat.name}"? Items will be moved to Uncategorized.`)) return;
      try {
...
document.getElementById('btn-cat-add').addEventListener('click', async () => {
  ...
  try {
    await api.addCategory({ name, color: catNewColor.value });
...
api.checkForUpdates().then((result) => {
  if (result.hasUpdate) {
    setTimeout(() => {
      ...
      banner.innerHTML = `Update v${result.version} available — <a href="#" id="update-link">Download</a>`;
      banner.querySelector('#update-link').addEventListener('click', (e) => {
        e.preventDefault();
        api.openUpdateUrl(result.url);
      });
      document.getElementById('title-bar').after(banner);
    }, 2000);
  }
});
